...
<script type="text/javascript">
	function readImage(input){//다운로드한 이미지 보기
	...
	function toList(obj){//글목록으로 돌아가기
		obj.action="${contextPath}/board/listArticles.do";
		obj.submit();
	}
	function fn_enable(obj){//비활성화 풀기 함수
	...
	function fn_modify_article(obj){//컨트롤러로 호출하는 함수
		obj.action="${contextPath}/board/modArticle.do";
		obj.submit();
	}
	function fn_remove_article(url, articleNo) {
		let form=document.createElement("form");//form태그를 만들어서 form이라는 변수에 넣음
		form.setAttribute("method","post");//속성세팅 ==> <form method="post">
		form.setAttribute("action",url);
		let articleNoInput=document.createElement("input");
		articleNoInput.setAttribute("type","hidden");
		articleNoInput.setAttribute("name","articleNo");
		articleNoInput.setAttribute("value",articleNo);//매개변수 값 받기
	...
	<form name="frmArticle" action="${contextPath }" method="post" enctype="multipart/form-data"><%--하나의 문서안에 폼이 여러개면 name을 꼭 줘서 구분해야함. --%>
		<table align="center">
			<tr>
				...
				<td><input type="text" value="${article.articleNo }" disabled></td><%--포워딩한게 넘어오는거임. name이 있어야 수정할때 갖고갈수있음.--%>
				<input type="hidden" name="articleNo" value="${article.articleNo }"><%-- boardController에 hidden으로 넘겨주기. --%>
			</tr>
			<tr>
